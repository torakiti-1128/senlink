...
### 2.4 認証責務の分担（CoreAPI / BFFAPI）
- Auth（ログイン、OTP、登録、ログアウト、トークン発行/更新）は **CoreAPI(Authサービス)** が提供する（発行元は一箇所）
- **BFFAPI と CoreAPI はどちらも JWT を検証する（二重検証）**
  - BFFAPI：統合/集計レスポンスのためのゲートウェイ（BFF）。受け取った Cookie(JWT) を検証してから各サービスへ連携する
  - CoreAPI：各ドメイン API を提供。外部から直接叩かれる可能性も踏まえ、必ず Cookie(JWT) を検証する
- 目的：
  - Apache 設定漏れ/経路追加/将来の構成変更があっても、CoreAPI 側で不正アクセスを防げる
  - BFF だけに認証を寄せるより安全で、運用破綻しにくい
- 実装方針：
  - JWT 検証ロジック（署名検証/期限/role 取り出し）は **共通モジュール化**して CoreAPI と BFFAPI で共有する（例：`shared/auth`）
